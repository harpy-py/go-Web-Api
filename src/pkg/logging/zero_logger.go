...
func newZeroLogger(conf *config.Config) *zeroLogger {
	logger := &zeroLogger{conf: conf}
	logger.Init()
	return logger
}

func (l *zeroLogger) getLogLevel() zerolog.Level {
	level, exists := zeroLogLevelMapping[l.conf.Logger.Level]
	if !exists {
...
func (l *zeroLogger) Init(){
	once.Do(func ()  {
		...
		file, err := os.OpenFile(l.conf.Logger.FilePath, os.O_CREATE| os.O_APPEND| os.O_WRONLY, 0666)
		if err != nil {
